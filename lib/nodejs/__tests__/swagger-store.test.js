...
describe('swagger-store', () => {
  ...
  describe('when called second time with same path', () => {
    it('should use cache', () => {
      const digest = swaggerStore.getDigest('/path/to/file');
      expect(digest).toEqual('DIGEST');
      expect(mockCrypto.createHash).not.toHaveBeenCalled();
      expect(mockCrypto.update).not.toHaveBeenCalled();
      expect(mockCrypto.digest).not.toHaveBeenCalled();
    });
  });
  ...
  it('should get swagger', () => {
    return swaggerStore
      ...
      .then((swagger) => {
        ...
        expect(swagger.swagger.swaggerObject).toBe('VALIDATED-DOC_EXCLUDE-HOST');
      });
  });
});
