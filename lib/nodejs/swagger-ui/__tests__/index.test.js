...
describe('swagger_ui', () => {
  ...
  const {swaggerUITag, swaggerUIProcessor} = require('../index')({hexo});
    ...
    test('should render one swagger ui instance, with all the libraries', async () => {
      ...
      await mockSwaggerUITag(['petstore.json'])
        .then((html) => {
          ...
          expect($('[input="swaggerJson"][input-type="json"][api-explorer="true"]').length).toBe(1);
          expect($('[ng-controller="swagger-ctrl-1"]').length).toBe(1);
          ...
          expect($('[src="/script/swagger/swagger-ui.min.js"]').length).toBe(1);
          expect($('[href="/style/swagger/swagger-ui-min.css"]').length).toBe(1);
        });
    });
  });
    ...
    test('should just render the swagger tag and skip the libraries', async () => {
      ...
      await mockSwaggerUITag(['petstore.json'])
        .then((html) => {
          ...
          expect($('[input="swaggerJson"][input-type="json"][api-explorer="true"]').length).toBe(1);
          expect($('[ng-controller="swagger-ctrl-2"]').length).toBe(1);
