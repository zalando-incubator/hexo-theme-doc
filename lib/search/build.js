...
module.exports = function build(ctx) {
  ...
  logger.info(`building search index and store for "${items.length}" items mapping to "${documents.length}" documents`);
...
function sanitize(html) {
  return striptags(html, [], ' ').trim() // strip html tags
    .replace(new RegExp('\n', 'g'), '') // removes linebreaks
    .replace(/ +(?= )/g,'') // removes double spaces
    .replace(/&#123;|&quot;|&#125;|&lt;|&gt;/g, '')
}

function url_for(path) {
  return '/' + path; // FIXME: use url_for helper
}
...
const PagesIndexer = {
  build: (ctx) => {
    const pages = Array.isArray(ctx.pages) ? ctx.pages : ctx.pages.data;
    const store = {};
    ...
    const documents = items.reduce((acc, page) => {
      ...
      $('h1,h2').each(function() {
        let elem = $(this);
        let path = `${page.path}#${elem.attr('id')}`;
